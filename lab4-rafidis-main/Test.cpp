...
// Test case
void testConstructor2Negative() {
	    int row=3,col=3;
        int data[3][3] = {{-1,-5,-3},{-10,-9,-4},{-4,-3,-4}};
        int*input_data[3];
        for (int i=0;i<3;i++){
        	input_data[i]=data[i];
        }
...
void testSetElementValid2() {
	// Test case
	  ...
	  int setvalid_row = 0, setvalid_col = 0;
	    int row=4,col=4;
	    int data[4][4] = {{1,2,3,4},{6,7,8,9},{0,1,2,3},{0,0,4,5}};
	    int*input_data[4];
	    for (int i=0;i<4;i++){
	    	input_data[i]=data[i];
	    }
...
void testAddToZeroes() {
	// Test case
	   int add_to_data[3][3] = {{3,4,5}, {1,2,3}, {0,0,2}}; // add to 'data' matrix, dimension not match
	   int add_to_above[3][3] = {{0,0,0}, {0,0,0}, {0,0,0}}; // add to 'add_to_data' matrix
	   int row=3;int col=3;
...
void testSubMatrixZero() {
// Test case
    int subvalid_row = 1, subvalid_col = 1;

	  int row=4,col=4;
      int data[4][4] = {{0,0,3,4},{0,0,5,6},{0,0,7,8},{0,0,9,0}};
	  int*input_data[4];
	  for (int i=0;i<4;i++){
	    	input_data[i]=data[i];
	  }
	  Matrix f =  Matrix(row,col,input_data);
		...
		try{
			...
			std::cerr << "This is supposed to be a valid input: " << e.what() << "\n";
...
bool runAllTests(int argc, char const *argv[]) {
	...
	s.push_back(CUTE(testConstructor1Boundary));

	s.push_back(CUTE(testConstructor2));
	s.push_back(CUTE(testConstructor2Negative));
